...
% Jacobi, Gauss-Seidel and SOR methods on a modified square grid
...
phi =1% phi0 * 4/(pi*sinh(pi)) * sin(pi*x'/L)*sinh(pi*y/L); 
...
saveas(gcf,'HW6_2a_contour','epsc');

figure(2); clf;
mesh(x,y,(phi));
xlabel('x'); ylabel('y'); zlabel('\Phi(x,y)');
saveas(gcf,'HW6_2a_mesh','epsc');
...
saveas(gcf,'HW6_2a_comptime','epsc');