...
x = 2*(0:N-1)*h;  % x coordinate
y = 2*(0:N-1)*h;  % y coordinate
eps0=1;
...
phi(round(N/2)+1:N,1:round(N/2)) = 0;  phi(:,1) = 0;  phi(:,N) = 0;
phi(1,:) = phi0*ones(N,1);    
Rho = zeros(N,N);
[ ~, iy ] = min( abs( y-0.5 ) ); %find the index of cell closest to y=0.5
[ ~, ix ] = min( abs( x ) );     %find the index of cell closest to x=0
Rho(iy,ix)=1/h;
phi=phi+Rho;

fprintf('Potential at y=0 equals %g \n',phi0);
     ...
     for j=2:(N-1)
       if i>round(N/2) && j<round(N/2)
            ...
            newphi = 0.25*omega*(phi(i+1,j)+phi(i-1,j)+ ...
               phi(i,j-1)+phi(i,j+1))  +  (1-omega)*phi(i,j)+1/eps0*h^2*Rho(i,j);
            changeSum = changeSum + abs(1-phi(i,j)/newphi);
       end
       phi(i,j) = newphi;
     end
...
saveas(gcf,'HW6_2b_contour','epsc');

figure(2); clf;
mesh(x,y,(phi));
xlabel('x'); ylabel('y'); zlabel('\Phi(x,y)');
saveas(gcf,'HW6_2b_mesh','epsc');
...
saveas(gcf,'HW6_2b_comptime','epsc');
