...
Diff(1,N) = 0;   Diff(1,1) = 0; Diff(1,2) = 0;
Diff(N,N-1) = 0; Diff(N,N) = 0; Diff(N,1) = 0;
...
drawnow; 
saveas(gcf,'HW6_1Dirichlet_init','epsc');
pause(1);
...
for iter=1:max_iter
  ...
  if( rem(iter,plot_iter) < 1 )   
    ...
    legend(sprintf('iter %g',iter));
    drawnow;
    saveas(gcf,'HW6_1Dirichlet_plots','epsc');
  end
...
saveas(gcf,'HW6_1Dirichlet_mesh','epsc');
...
saveas(gcf,'HW6_1Dirichlet_selplots','epsc');