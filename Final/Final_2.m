%  schro - Program to solve the Schrodinger equation for a delta potential
...
%Center must include the delta function
ham(N/2,N/2)=-2*coeff+u*1/h^2;
...
k0 = 1;       % Average wavenumber. Recall p=\hbar k, since \hbar=1 p, k are equivalent
...
max_iter = 1/16*L/(tau^2);      % Particle should circle system
plot_iter = max_iter/100;          % Produce 20 curves
...
ref=trapz(x(1:length(x)/2),p_plot(1:length(x)/2,100))
trans=trapz(x(length(x)/2:length(x)),p_plot(length(x)/2:length(x),100))